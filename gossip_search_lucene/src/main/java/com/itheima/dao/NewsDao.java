...
public class NewsDao extends JdbcTemplate {
  ...
  public List<News> queryList(int currentPage, int rows) {
    ...
    String sql = " select id,title,docurl,content,editor,time,source from news limit ? , ?";
    List<News> list = query(sql, new BeanPropertyRowMapper<News>(News.class), currentPage, rows);
