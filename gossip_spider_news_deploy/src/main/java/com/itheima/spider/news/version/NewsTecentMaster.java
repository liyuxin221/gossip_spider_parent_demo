...
/**
 * @author: Liyuxin wechat:13011800146. @Title: NewsTecentMaster @ProjectName gossip_spider_parent
...
public class NewsTecentMaster {
    ...
    private static void parsedTecentNews(String hotNewsUrl, String normalNewsUrl) throws IOException {
        ...
        while (true) {
            ...
            // 将newsJson保存到newsList
            saveNewsList(normalNewsList);
     ...
     */
    private static void saveNewsList(List<News> hotNewsList) {
        for (News news : hotNewsList) {
            // 判断是否已抓取
            Jedis jedis = JedisUtils.getJedis();
            String newsJson = gson.toJson(news, News.class);
